...
export default function RoomChat({ roomId, isMember }: RoomChatProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-background/40 backdrop-blur-xl">
            {/* Header */}
            <div className="p-4 sm:p-6 border-b border-primary/5 bg-primary/[0.03] backdrop-blur-md">
                <h3 className="text-xs sm:text-sm font-black text-foreground flex items-center uppercase tracking-widest">
                    <span className={`w-2 h-2 rounded-full mr-3 animate-pulse ${socketRef.current?.readyState === 1 ? 'bg-green-500' : 'bg-red-500'}`}></span>
                    Room Chat
                </h3>
            </div>

            {/* Messages List */}
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 custom-scrollbar pb-10">
                {messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                        ...
                        >
                            <div className="flex items-baseline space-x-2 mb-1 px-1">
                                <span className={`text-[9px] sm:text-[10px] font-black uppercase tracking-widest font-sans ${msg.user_id === user?.id ? 'text-primary' : 'text-foreground/40'}`}>
                                    {msg.owner.username}
                                </span>
                                <span className="text-[8px] text-foreground/20 font-sans">
                                    {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </span>
                            </div>
                            <div
                                className={`px-4 py-2 sm:py-2.5 rounded-[1rem] sm:rounded-[1.2rem] max-w-[95%] sm:max-w-[85%] break-words text-xs sm:text-sm font-sans shadow-sm border ${msg.user_id === user?.id
            ...
            {/* Rate Limit / Error Warning */}
            {
                error && !rateLimitTimer && (
                    <div className="mx-4 sm:mx-6 mb-4 px-4 py-3 bg-destructive/10 text-destructive text-[9px] font-black uppercase tracking-widest font-sans flex items-center rounded-xl border border-destructive/20">
            ...
            {/* Input Area */}
            <div className="p-4 sm:p-6 bg-primary/[0.03] border-t border-primary/5 mt-auto">
                <form onSubmit={handleSendMessage} className="relative">
                    <input
                        type="text"
                        value={newMessage}
                        onChange={(e) => setNewMessage(e.target.value)}
                        placeholder={!isMember ? "Join to participate..." : (rateLimitTimer ? `Wait ${rateLimitTimer}s...` : "Type a message...")}
                        className={`w-full bg-background/50 border border-primary/10 rounded-full pl-5 sm:pl-6 pr-12 sm:pr-14 py-3 sm:py-4 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all font-sans ${rateLimitTimer || !isMember ? 'cursor-not-allowed opacity-50' : ''}`}
                        disabled={rateLimitTimer !== null || !isMember}
                    />
                    <button
                        type="submit"
                        disabled={!newMessage.trim() || rateLimitTimer !== null}
                        className={`absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 h-10 flex items-center justify-center rounded-full transition-all ${!newMessage.trim() || rateLimitTimer !== null
                            ? 'text-foreground/20'
                            : 'text-white bg-primary shadow-lg shadow-primary/20 hover:scale-105 active:scale-95'
                            }`}
                    >
                        {rateLimitTimer ? (
                            <span className="text-[9px] sm:text-[10px] font-black font-sans">{rateLimitTimer}</span>
                        ) : (
