    ...
    return (
        <div className="flex gap-4 group/post">
            ...
            <div className="flex flex-col items-center space-y-2 pt-4">
                <button
                    onClick={handleLike}
                    className={`p-1.5 rounded-lg transition-all duration-300 ${isLiked ? 'bg-primary/10 text-primary' : 'text-foreground/20 hover:text-primary hover:bg-primary/5'}`}
                >
                    <Heart className={`w-5 h-5 ${isLiked ? 'fill-current' : ''}`} />
                </button>
                <span className={`text-xs font-black font-sans ${isLiked ? 'text-primary' : 'text-foreground/40'}`}>
                    {likesCount}
                </span>
