            ...
            {isEditingPost ? (
                <div className="mb-6 space-y-4">
                    <input
                        ...
                        className="w-full bg-background/50 border border-primary/20 rounded-xl px-4 py-2 text-xl font-bold focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                    <textarea
                        value={editPostContent}
                        onChange={e => setEditPostContent(e.target.value)}
                        className="w-full bg-background/50 border border-primary/20 rounded-xl px-4 py-2 min-h-[100px] focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                    <div className="flex gap-2 justify-end">
                        <button type="button" onClick={() => setIsEditingPost(false)} className="p-2 rounded-full hover:bg-red-500/10 text-red-500"><X className="w-5 h-5" /></button>
                        <button type="button" onClick={handleUpdatePost} className="p-2 rounded-full hover:bg-green-500/10 text-green-500"><Check className="w-5 h-5" /></button>
                    </div>
                </div>
            ) : (
                <>
                    <div className="flex justify-between items-start mb-4">
                        ...
                        {isPostOwner && (
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <button onClick={() => setIsEditingPost(true)} className="p-2 text-foreground/30 hover:text-primary transition-colors">
                                    <Pencil className="w-4 h-4" />
                                </button>
                                <button onClick={handleDeletePost} className="p-2 text-foreground/30 hover:text-destructive transition-colors">
                    ...
                    {post.attachment_url && (
                        <div className="mb-6">
                            <a
                                ...
                                className="inline-flex items-center gap-2 px-4 py-2 bg-primary/5 text-primary rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-primary/10 transition-colors border border-primary/10"
                            >
